...
Con la creciente tendencia de la potencia de cálculo en los dispositivos actuales, prácticamente todos los aparatos electrónicos que nos rodean han crecido en cuanto a potencia y complejidad de cálculo. Uno de ellos, los teléfonos móviles, son con diferencia el dispositivo más extendido, y una herramienta potencial para utilizar en tareas diarias.\\

Éstos, basados en arquitectura ARM, se basan en la idea de ofrecer el máximo rendimiento posible dentro de unos consumos contenidos, para evitar problemas de autonomía o de temperatura, por lo que el entrenamiento de una red relativamente pequeña de menos de 10 millones de parámetros podría ser una tarea compleja. Sin embargo, en el caso de la inferencia, estos pueden llegar a ser muy rápidos, y podemos aprovechar la capacidad que ofrecen para procesar nuestro entorno en base a modelos preentrenados.\\

Esta tarea ya se realiza a través de herramientas como Google Lens, u otro tipo de aplicaciones que incorporan capacidades de visión por computador, como Google Maps, y se trata de un campo en auge. Su aprovechamiento, como ya mencionamos en la introducción, podría facilitarnos la inferencia del problema que nos concierne, evitando la necesidad de enviar los datos a un servidor externo, y que estos regresen de vuelta.\\

En este punto, estudiaremos algunos modelos comúnmente usados, y algunas aplicaciones concretas aplicadas en medicina y el cáncer de piel.

\subsection{Arquitecturas de redes convolucionales}

A lo largo de las última década, se han desarrollado numerosos modelos de red para smartphones, inicialmente como versiones ``recortadas'' de los modelos tradicionales de computador, y posteriormente, éstas se crearon específicamente con ese propósito, limitando el número de parámetros y capas.
...
Esta arquitectura introduce varias innovaciones que permiten reducir su tamaño, e incluo aumentar levemente el rendimiento obtenido con respecto a Alex-Net. Los tres más destacados son:
\begin{itemize}
...
Dos años más tarde de la primera versión, se da a luz a su versión V2. Esta conserva los hiperparámetros  de la versión anterior, así como el producto punto a punto. Sin embargo, añade tres nuevas características, algunas de ellas no triviales y que requieren experimentación:
\begin{itemize}
