\chapter{Análisis y estudio de modelos sobre la base de datos}
\label{sec:capt5}
Tras la descripción de los modelos a utilizar, y las funciones de pérdida a emplear, comienza el proceso de entrenamiento de los modelos. Este proceso es uno de los más lentos de todo el desarrollo del proyecto, debido a la cantidad de horas necesarias a esperar para recopilar los resultados del modelo. Se estudiará la eficacia de cada modelo a la hora de evaluar el problema, y las decisiones tomadas para su mejora y elección.
...
Un aspecto clave de este proceso será la eficiencia, pero sobre todo el correcto diagnóstico de las enfermedades, por lo que usaremos como métricas de selección precision, recall y balanced accuracy. Las 3 se basan en las propiedades de la matriz de confusión.

La matriz de confusión es una herramienta que nos permite evaluar el modelo entrenado mediante la definición de 4 posibles estados de la clasificación, tal y como se describe en la figura \ref{fig:confusionmatrix}. Se basa en la relación que existe entre el valor real y el valor predicho, teniendo en cuenta si se produjo acierto o error.

\begin{figure}[H]
	\centering
	\includegraphics[scale = 0.6]{imagenes/confmat.png}
	\caption{Definición de la matriz de confusión, extraído de \cite{confmat}}
...
\end{itemize}
 ...
 \begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\Comment{Comprueba si la carpeta contenedora es validación}
...
Otra gran ventaja de este método es que no se requieren gran cantidad de parámetros. Con la implementación realizada en FastAI, solo es necesario especificar el número de épocas a realizar, y un valor máximo de learning rate, clave para especificar el valor pico de la fase ascendente del learning rate.
