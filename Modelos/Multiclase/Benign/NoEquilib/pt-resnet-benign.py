...
train_benign = pd.read_csv('train_benign.csv')
...
# %%
"""
train_benign = trainSet.loc[trainSet['bin'] == 0]
train_benign['label'] = train_benign['label'].replace(
    {'solar_lentigo': 'lentigo', 'lentigo_nos': 'lentigo', 'lentigo_simplex': 'lentigo'})
train_benign.to_csv('train_benign.csv', index=False)
"""

# %%
print(train_benign.label.value_counts())
sns.countplot(train_benign, x='label', order=train_benign.label.value_counts().index)
...
# %%
"""
train_imgs_names = train_benign['image'].to_list()
mover_imgs(train_imgs_names, 'benignThumbnails')"""

# %%
def multi_label_benign(fname):
    global train_benign
    el = train_benign.loc[TRAIN_DIR + train_benign['image'] == str(fname)]
...
train_ds = DataBlock(
    ...
    get_y=multi_label_benign,  # Las etiquetas son especificadas como contenido del dataset en si
    item_tfms=[Resize(IMAGE_RESIZE, method=RESIZE_METHOD), AlbumentationsTransform(get_train_aug())],  # Redimensionado
    batch_tfms=Normalize.from_stats(*imagenet_stats)
).dataloaders("benignThumbnails/", bs=BATCH_SIZE)
...
save_callback = SaveModelCallback(monitor='valid_loss', fname='best_model_benign')
...
plt.savefig("perdidas50_freeze_benign.png")
...
# %%
perdidas = learn_resnet.recorder.plot_loss(skip_start=0, with_valid=True)
plt.savefig("perdidas50_unfreeze_benign.png")
...
# %%
learn_resnet.save('modelo_fastai_bestISICbenign')
...
scripted_module.save("bestISICbenign_512.pt")
# Export mobile interpreter version model (compatible with mobile interpreter)
optimized_scripted_module = optimize_for_mobile(scripted_module)
optimized_scripted_module._save_for_lite_interpreter("bestISICbenign512_android.ptl")
...
scripted_module = torch.jit.script(nn.Sequential(model, nn.Softmax(dim=1)))
# Export full jit version model (not compatible mobile interpreter), leave it here for comparison
scripted_module.save("bestISICbenign_512_noop.pt")
# Export mobile interpreter version model (compatible with mobile interpreter)
scripted_module._save_for_lite_interpreter("bestISICbenign 512_android_noop.ptl")
